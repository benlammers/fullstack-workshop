...
const PostItem = ({ post, removePost }) => {
   ...
   const { data: comments } = useQuery(["comments", post._id], () => fetchComments(post._id));
   ...
   let showDelete = author === username && removePost !== null;

   return (
      <Card
         title={<Link to={`/post/${_id}`}>{title}</Link>}
         className='post-item'
         extra={[<span key='date'>{formattedDate}</span>, showDelete && <DeleteOutlined key='delete' style={{ fontSize: "24px" }} onClick={() => removePost(_id)} />]}
      >
